...
public class RangeSliderTrackLayer: CALayer {
    ...
    override open func draw(in ctx: CGContext) {
        ...
        // Clip
        let newHeight = bounds.height/3 // Use this to change height of slider "bounds.height" is half of thumb
        let yPos = bounds.minY + bounds.height/2 - newHeight/2
        let newRect = CGRect(x: bounds.minX, y:yPos , width: bounds.width, height: newHeight)
        let cornerRadius = newRect.height * slider.curvaceousness / 2.0
        ...
        let rect = CGRect(x: lowerValuePosition, y: yPos, width: upperValuePosition - lowerValuePosition, height: newHeight)
        ctx.fill(rect)
    }
}
